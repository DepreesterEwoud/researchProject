...
class DDQNAgent(object):
    def __init__(self, alpha, gamma, n_actions, epsilon, batch_size,
                 ...
                 mem_size=25000, fname='ddqn_model_test.h5', replace_target=25):
    ...
    def learn(self):
        ...
        if self.memory.mem_cntr > self.batch_size:
            ...
            y = self.brain_eval.predict(state)
            #print(y)

            next_state_q_values = self.brain_target.predict(new_state)
            ...
            for i in range(self.batch_size):
                if done[i]:
                    ...
                    y[i,action[i]] = reward[i] + DISCOUNT *  max_q_next_state[i]
            #print("hallo")
            self.brain_eval.train(state, y, self.batch_size, 0)
...
class Brain:
    ...
    def createModel(self):
        ...
        model.add(tf.keras.layers.Dense(128, activation=tf.nn.relu)) #prev 256 
