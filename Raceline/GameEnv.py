...
from RaceLine import getRaceLines
...
class RacingEnv:
    ...
    def step(self, action):
        ...
        # Check if car is racing ideal racing line
        indexrace = 1
        for line in self.racelines:
            if indexrace > len(self.racelines):
                indexrace = 1
            if line.isactiv:
                if self.car.collision(line):
                    line.isactiv = False
                    self.racelines[indexrace-2].isactiv = True
                    reward += 3
